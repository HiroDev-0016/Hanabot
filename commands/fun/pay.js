...
db.set(`id_${message.author.id}`, user.id)
const id = await db.fetch(`id_${message.author.id}`)
...
${user.tag} Receberá ${args[2]}, ficando com ${argsCheck}

Para a transação ser completada você deve reagir em ✅ e <@${user.id}> deve reagir em 👍**`).then(msg => {
msg.react('✅')
msg.react('👍')
            let filtro2 = (reaction, usuario) => reaction.emoji.name === "✅" && usuario.id === message.author.id
            let coletor2 = msg.createReactionCollector(filtro2, {max: 999})
            coletor2.on("collect", cp => {
              if(db.fetch(`userreact_${user.id}`) === 1) {
              msg.delete()
              db.add(`credits_${user.id}`, args[2])
              db.subtract(`credits_${message.author.id}`, args[2])
              const credits3 = db.fetch(`credits_${user.id}`)
  db.set(`authorreact_${message.author.id}`, 0)
 db.set(`userreact_${user.id}`, 0)
            return message.channel.send(`<a:hana_certo:699014500594090115> ┃ Dinheiro enviado com sucesso! Agora, ${user.tag} tem ${credits3} créditos!`)
}
 db.set(`authorreact_${message.author.id}`, 1)              
            })

            let filtro3 = (reaction, usuario) => reaction.emoji.name === "👍" && usuario.id === id
            let coletor3 = msg.createReactionCollector(filtro3, {max: 999})
            coletor3.on("collect", cp => {
              if(db.fetch(`authorreact_${message.author.id}`) === 1) {
              msg.delete()
              db.add(`credits_${user.id}`, args[2])
              db.subtract(`credits_${message.author.id}`, args[2])
              const credits3 = db.fetch(`credits_${user.id}`)
  db.set(`authorreact_${message.author.id}`, 0)
 db.set(`userreact_${message.author.id}`, 0)
            return message.channel.send(`<a:hana_certo:699014500594090115> ┃ Dinheiro enviado com sucesso! Agora, ${user.tag} tem ${credits3} créditos!`)
}
 db.set(`userreact_${user.id}`, 1)              
            })
})
